	...
	                    });
					...
					case 'reply_question':
						...
						if (result.rsm.ajax_html)
						{
							...
							$('.aw-question-comment').find('.aw-nav-tabs').find('h2.d-none d-sm-block').html((parseInt($('.aw-question-comment').find('.aw-nav-tabs').find('h2.d-none d-sm-block').html())+1)+' 个回复');
...
AWS.Dropdown =
{
	...
	bind_dropdown_list: function(selector, type)
	{
		if (type == 'search')
		{
			...
			{
				$(selector).parent().find('.dropdown-menu').show();
			});
		}
		$(selector).bind('input propertychange',function(e)
		{
			...
			{
			   $(selector).parent().find('.dropdown-menu').hide();
			}
			...
			if (type == 'topic')
			{
				...
				if (e.which == 188)
				{
					if ($('.aw-edit-topic-box #aw_edit_topic_title').val() != ',')
					{
						...
						$('.aw-edit-topic-box .dropdown-menu').hide();
		...
		{
			$(selector).parent().find('.dropdown-menu').delay(500).fadeOut(300);
		});
	},
	...
	get_dropdown_list: function(selector, type, data)
	{
		...
		AWS.G.dropdown_list_xhr = $.get(url, function (result)
		{
			if (result.length != 0 && AWS.G.dropdown_list_xhr != undefined)
			{
				...
				{
					$(selector).parent().find('.dropdown-menu, .aw-dropdown-list').show().children().show();
			...
			{
				$(selector).parent().find('.dropdown-menu').show().end().find('.title').html(_t('没有找到相关结果')).show();
...
(function ($)
{
	$.fn.extend(
	{
		...
		/* 快版修改 因autosize插件升级到最新版，调用方法变化，这里补一个jquery方法定义 ，为兼容老版 */
		autosize: function()
		{
			autosize($(this));
		}
